...
public class UsersDAO {
    ...
    public MidUser findByUserName(String userName) {
    	return (MidUser) midasEntityManager.createNamedQuery("MidUser.findByUserName").setParameter("userName", userName).getSingleResult();
    }
    ...
    public MidUser addUserRole(MidUser user, MidRole role) {
    	try {
            ...
            log.error("Error añadiendo rol " + role.getRoleId() + " al  usuario " + user.getUserName(), e);
            user = null;
    	}
    	return user;
    }
    ...
    public MidUser deleteUserRole(MidUser user, MidRole role) {
    	try {
            ...
            log.error("Error eliminando rol " + role.getRoleId() + " al  usuario " + user.getUserName(), e);
            user = null;
    	}
    	return user;
    }   
    ...
    public MidUser addUserContext(MidUser user, MidContext context) {
    	try {
    		if (!user.getMidContextList().contains(context)) {
    			...
    			log.debug("Añadiendo contexto " + context.getCxId() + " al  usuario " + user.getUserName());
    			midasEntityManager.merge(context);
    		} else {
    			log.warn("No se añade contexto preasignado");
    		}
    	} catch (Exception e) {
            log.error("Error añadiendo contexto " + context.getCxId() + " al  usuario " + user.getUserName(), e);
            user = null;
    	}
    	return user;
    }
    ...
    public MidUser deleteUserContext(MidUser user, MidContext context) {
    	try {
            ...
            log.error("Error eliminando contexto " + context.getCxId() + " al  usuario " + user.getUserName(), e);
            user = null;
    	}
    	return user;
    }
